...
def fill_lesson_form(form: LessonForm):
    form.category_id.choices = [(category.id, f"ID: {category.id} - {category.title}") for category in Category.query.all()]
    form.level_id.choices = [(level.id, f"ID: {level.id} - {level.title}({level.indicator})") for level in Level.query.all()]
